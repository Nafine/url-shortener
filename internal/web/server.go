...
func New(cfg *config.Config, apiKeys map[string]string, log *slog.Logger, db *postgres.Storage) *Server {
	if cfg.AppEnv == "prod" {
	...
	return &Server{
		...
		router:  newRouter(log, db, apiKeys),
	}
}

func newRouter(log *slog.Logger, db *postgres.Storage, apiKeys map[string]string) *gin.Engine {
	router := gin.New()
...

	{
		...
		root.POST("", middleware.JSONParser[handler.SaveRequest](log), handler.Save(log, db))
		{
			...
			authoritative.Use(middleware.APIKeyAuth(log, apiKeys))
